...
export class UsuarioController {
  ...
  public editarUsuario = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const usuario = await this.usuarioService.editarUsuario(id, {
        email,
        nombre,
        apellido,
        direccion,
  ...
  public recuperarClave = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(400)
        .json({ error: error.message || "Error al recuperar la clave" });
    }
  };
  ...
  public renovarClave = async (req: Request, res: Response) => {
    try {
      ...
      const resultado = await this.usuarioService.renovarClaveConToken(
        token,
        password,
      );
      ...
      return res
        .status(400)
        .json({ error: error.message || "Error al renovar  la clave" });
    }
  };
}
